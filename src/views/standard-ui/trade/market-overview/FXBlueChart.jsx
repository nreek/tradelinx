...
export class FXBlueChart extends Component {
  ...
  setChartTimeframe = (timeframe) => {
    this.setState({ timeframe }, () => {
      ...
      this.fxblueChartConfig.charts[0].instrumentDef.symbol = `bitfinex@${
        this.props.selectedInstrument.id
      }`;
      this.fxblueChartConfig.charts[0].instrumentDef.symbolCaption = `${
        this.props.selectedInstrument.id
      }`;
  ...
  render() {
    ...
    const url = `${this.state.urlSource}?css=//${window.location.host}/${
      fxblueChartConfig.cssUrl
    }&${this.chartUrlParam(this.props.selectedInstrument.id)}&exchangeID=${
      this.props.config.chartConfig.clientId
    }&userID=${userToken}`;

    return (
      <div className="fxblue-chart">
        <div className="chart-timeframe-selector">{frames}</div>
        <Iframe src={url} height="400px" id="fxblue-chart">
