...
import { selectPasswordResetRequestStatus } from '../../reducers/user';
...
class ResetPasswordRequest extends Component {
  ...
  renderRequestFailed = () => (
    <Fragment>
      <p className='password-reset-input-instructions error'>
  ...
  renderRequestSuccess = () => {
    if (this.props.passwordResetRequestStatus === status.verify) {
      ...
      if (changePasswordWithCodeStatus === status.success) {
        return (
          <Fragment>
            <p className='password-reset-input-instructions success'>{_t(
              'Your password was successfully changed. Please',
              'RESET_PASSWORD.CHANGED_1'
            )}
              <a href="/login">{_t(
                'Log In',
                'RESET_PASSWORD.LOG_IN'
              )}</a>
            {_t(
              'to continue.',
