...
const OrderRow = ({ data }) => (
  <div className="table-row inactive-orders-row">
    <span className="inactive-orders-account"><CopyToClipboard text={data.order_id} length={4} /></span>
    <span className="inactive-orders-instrument">{data.security_id}</span>
    <span className={`inactive-orders-side ${data.order_side}`}>
      {_t(
        data.order_side.toUpperCase(),
        `ORDER_TABLES.SIDE_${data.order_side.toUpperCase()}`,
      )
                }
    </span>
    <span className="inactive-orders-status">
      {_t(
        data.order_status,
        `ORDER_TABLES.STATUS_${data.order_status.toUpperCase()}`,
      )
                }
    </span>
    <span className="inactive-orders-quantity">{data.reason}</span>
    <span className="inactive-orders-time">{getFormattedDate(data.timestamp)}</span>
  </div>
);
        ...
        <Fragment>
          <div className="table inactive-orders-table">
            <div className="table-header inactive-orders-header">
              <span className="inactive-orders-account">{_t('#', 'ORDER_TABLES.ORDER_ID')}</span>
              ...
              <span className="inactive-orders-quantity">{_t('Reason', 'ORDER_TABLES.REASON')}</span>
...
const mapStateToProps = state => ({
  data: selectInactiveOrders(state).slice().reverse(), // Makes copy and reverses
